...
const queryListStoreRulePickingActionsE = `select uniqueid, sede, flag1, flag2, rulepickzoneid, orden, warehousetext, storagetypetext, locationtypeenumid, locationstatusid, permanent, producttypeid, categitemtext, inventorytypeid, statusitemid, umedida, activo, estadoreg, total_records from store_rules_pick_actions_list( $1, $2)`
const queryLoadStoreRulePickingActionsE = `select * from store_rules_pick_actions_list( $1, $2)`
...
func (u *StoreRulePickingActionsE) GetAll(token string, filter string) ([]*StoreRulePickingActionsE, error) {
	...
	///uniqueid, sede, flag1, flag2, rulepickzoneid, orden, warehousetext, storagetypetext, locationtypeenumid,
...
func (u *StoreRulePickingActionsE) GetByUniqueid(token string, jsonText string) (*StoreRulePickingActionsE, error) {
	ctx, cancel := context.WithTimeout(context.Background(), dbTimeout)
	defer cancel()

	query := queryLoadStoreRulePickingActionsE
