...
func (u *StoreWarehouseE) GetAll(token string, filter string) ([]*StoreWarehouseE, error) {
	ctx, cancel := context.WithTimeout(context.Background(), dbTimeout)
	defer cancel()

	query := queryListStoreWarehouseE
	...
	var lista []*StoreWarehouseE

	///uniqueid, sede, flag1, flag2, secuencial, code, descrip, activo, estadoreg, total_records
	for rows.Next() {
		var rowdata StoreWarehouseE
		err := rows.Scan(
			&rowdata.Uniqueid,
			&rowdata.Sede,
			&rowdata.Flag1,
...
func (u *StoreWarehouseE) GetByUniqueid(token string, jsonText string) (*StoreWarehouseE, error) {
	ctx, cancel := context.WithTimeout(context.Background(), dbTimeout)
	defer cancel()

	query := queryLoadStoreWarehouseE
...
func (u *StoreWarehouseE) Update(token string, data string, metricas string) (map[string]any, error) {
	ctx, cancel := context.WithTimeout(context.Background(), dbTimeout)
	defer cancel()
	...
	query := querySaveStoreWarehouseE
	stmt, err := db.Prepare(query)
	if err != nil {
		return nil, err
	}
...
func (u *StoreWarehouseE) Delete(token string, data string, metricas string) (map[string]any, error) {
	ctx, cancel := context.WithTimeout(context.Background(), dbTimeout)
	defer cancel()
...
func (u *StoreWarehouseE) DeleteByID(token string, id int, metricas string) (map[string]any, error) {
	ctx, cancel := context.WithTimeout(context.Background(), dbTimeout)
	defer cancel()
