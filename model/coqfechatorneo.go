...
type CoqFechaTorneoE struct {
	...
	FechaNumero      NullInt32   `json:"fecha_numero,omitempty"`
	FechaDia         NullString  `json:"fecha_dia,omitempty"`
	...
	EstadoBefore     NullInt64   `json:"estado_before,omitempty"`
...
func (e CoqFechaTorneoE) MarshalJSON() ([]byte, error) {
	return MarshalJSON_Not_Nulls(e)
}

const querySelectCoqFechaTorneo = `select * from coq_fecha_torneo_list( $1, $2)`
...
func (u *CoqFechaTorneoE) GetAll(token string, filter string) ([]*CoqFechaTorneoE, error) {
	ctx, cancel := context.WithTimeout(context.Background(), dbTimeout)
	defer cancel()

	query := querySelectCoqFechaTorneo
	...
	var lista []*CoqFechaTorneoE

	for rows.Next() {
		var rowdata CoqFechaTorneoE
		err := rows.Scan(
			&rowdata.Uniqueid,
			//&rowdata.Owner,
...
func (u *CoqFechaTorneoE) GetByUniqueid(token string, uniqueid int) (*CoqFechaTorneoE, error) {
	ctx, cancel := context.WithTimeout(context.Background(), dbTimeout)
	defer cancel()
...
func (u *CoqFechaTorneoE) Update(token string, data string, metricas string) (map[string]any, error) {
	ctx, cancel := context.WithTimeout(context.Background(), dbTimeout)
	defer cancel()
	...
	query := `SELECT coq_fecha_torneo_save($1, $2, $3)`
	stmt, err := db.Prepare(query)
	if err != nil {
		return nil, err
	}
...
func (u *CoqFechaTorneoE) Delete(token string, data string, metricas string) (map[string]any, error) {
	ctx, cancel := context.WithTimeout(context.Background(), dbTimeout)
	defer cancel()
...
func (u *CoqFechaTorneoE) DeleteByID(token string, id int, metricas string) (map[string]any, error) {
	ctx, cancel := context.WithTimeout(context.Background(), dbTimeout)
	defer cancel()
