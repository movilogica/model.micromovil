...
// Personas Medios
type DataPersonasMedioE struct {
	...
	TokenMedio    NullString `json:"tokenmedio,omitempty"`
	ChannelTypeId NullInt64  `json:"channel_type_id,omitempty"`
	...
	StatusChannel NullInt32  `json:"status_channel,omitempty"`
...
func (e DataPersonasMedioE) MarshalJSON() ([]byte, error) {
	return MarshalJSON_Not_Nulls(e)
}

const querySelectDataPerMedios = `select * from data_personas_medios_list( $1, $2)`
...
func (u *DataPersonasMedioE) GetAll(token string, filter string) ([]*DataPersonasMedioE, error) {
	ctx, cancel := context.WithTimeout(context.Background(), dbTimeout)
	defer cancel()

	query := querySelectDataPerMedios
	...
	var lista []*DataPersonasMedioE

	for rows.Next() {
		var rowdata DataPersonasMedioE
		err := rows.Scan(
			&rowdata.Uniqueid,
			&rowdata.Owner,
...
func (u *DataPersonasMedioE) GetByUniqueid(token string, uniqueid int) (*DataPersonasMedioE, error) {
	ctx, cancel := context.WithTimeout(context.Background(), dbTimeout)
	defer cancel()
...
func (u *DataPersonasMedioE) Update(token string, data string, metricas string) (map[string]any, error) {
	ctx, cancel := context.WithTimeout(context.Background(), dbTimeout)
	defer cancel()
	...
	query := `SELECT data_personas_medios_save($1, $2, $3)`
	stmt, err := db.Prepare(query)
	if err != nil {
		return nil, err
	}
...
func (u *DataPersonasMedioE) Delete(token string, data string, metricas string) (map[string]any, error) {
	ctx, cancel := context.WithTimeout(context.Background(), dbTimeout)
	defer cancel()
...
func (u *DataPersonasMedioE) DeleteByID(token string, id int, metricas string) (map[string]any, error) {
	ctx, cancel := context.WithTimeout(context.Background(), dbTimeout)
	defer cancel()
