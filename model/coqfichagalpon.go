...
func (e CoqFichaGalponE) MarshalJSON() ([]byte, error) {
	return MarshalJSON_Not_Nulls(e)
}

const querySelectCoqFichaGalpon = `select * from coq_ficha_galpon_list( $1, $2)`
...
func (u *CoqFichaGalponE) GetAll(token string, filter string) ([]*CoqFichaGalponE, error) {
	ctx, cancel := context.WithTimeout(context.Background(), dbTimeout)
	defer cancel()

	query := querySelectCoqFichaGalpon
	...
	var lista []*CoqFichaGalponE

	for rows.Next() {
		var rowdata CoqFichaGalponE
		err := rows.Scan(
			&rowdata.Uniqueid,
			//&rowdata.Owner,
...
func (u *CoqFichaGalponE) GetByUniqueid(token string, uniqueid int) (*CoqFichaGalponE, error) {
	ctx, cancel := context.WithTimeout(context.Background(), dbTimeout)
	defer cancel()
...
func (u *CoqFichaGalponE) Update(token string, data string, metricas string) (map[string]any, error) {
	ctx, cancel := context.WithTimeout(context.Background(), dbTimeout)
	defer cancel()
	...
	query := `SELECT coq_ficha_galpon_save($1, $2, $3)`
	stmt, err := db.Prepare(query)
	if err != nil {
		return nil, err
	}
...
func (u *CoqFichaGalponE) Delete(token string, data string, metricas string) (map[string]any, error) {
	ctx, cancel := context.WithTimeout(context.Background(), dbTimeout)
	defer cancel()
...
func (u *CoqFichaGalponE) DeleteByID(token string, id int, metricas string) (map[string]any, error) {
	ctx, cancel := context.WithTimeout(context.Background(), dbTimeout)
	defer cancel()
