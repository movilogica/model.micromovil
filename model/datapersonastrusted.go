...
// Personas Trusted
type DataPersonaTrustedE struct {
	...
	ParentType    NullString  `json:"parentype,omitempty"`
	...
	MaximunAmount NullFloat64 `json:"maximum_amount,omitempty"`
	...
	StatusTrusted NullInt32   `json:"status_trusted,omitempty"`
...
func (e DataPersonaTrustedE) MarshalJSON() ([]byte, error) {
	return MarshalJSON_Not_Nulls(e)
}

const querySelectDataPerTrust = `select * from data_personas_trusted_list( $1, $2)`
...
func (u *DataPersonaTrustedE) GetAll(token string, filter string) ([]*DataPersonaTrustedE, error) {
	ctx, cancel := context.WithTimeout(context.Background(), dbTimeout)
	defer cancel()

	query := querySelectDataPerTrust
	...
	var lista []*DataPersonaTrustedE

	for rows.Next() {
		var rowdata DataPersonaTrustedE
		err := rows.Scan(
			&rowdata.Uniqueid,
			&rowdata.Owner,
...
func (u *DataPersonaTrustedE) GetByUniqueid(token string, uniqueid int) (*DataPersonaTrustedE, error) {
	ctx, cancel := context.WithTimeout(context.Background(), dbTimeout)
	defer cancel()
...
func (u *DataPersonaTrustedE) Update(token string, data string, metricas string) (map[string]any, error) {
	ctx, cancel := context.WithTimeout(context.Background(), dbTimeout)
	defer cancel()
	...
	query := `SELECT data_personas_trusted_save($1, $2, $3)`
	stmt, err := db.Prepare(query)
	if err != nil {
		return nil, err
	}
...
func (u *DataPersonaTrustedE) Delete(token string, data string, metricas string) (map[string]any, error) {
	ctx, cancel := context.WithTimeout(context.Background(), dbTimeout)
	defer cancel()
...
func (u *DataPersonaTrustedE) DeleteByID(token string, id int, metricas string) (map[string]any, error) {
	ctx, cancel := context.WithTimeout(context.Background(), dbTimeout)
	defer cancel()
