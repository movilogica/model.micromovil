...
const querySelectStoreParamCategItem = `select * from store_param_categ_items_list( $1, $2)`
...
func (u *StoreParamCategItemE) GetAll(token string, filter string) ([]*StoreParamCategItemE, error) {
	...
	query := querySelectStoreParamCategItem
...
func (u *StoreParamCategItemE) Update(token string, data string, metricas string) (map[string]any, error) {
	...
	query := `SELECT store_param_categ_items_save($1, $2, $3)`
	stmt, err := db.Prepare(query)
	if err != nil {
		return nil, err
	}
